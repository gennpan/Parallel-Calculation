...
#define CHUNK 10 //aumentando questo valore lo schedule tende ad essere static mentre diminuendolo tende ad essere di tipo dynamic.

//FUNZIONE PER ESTRARRE GLI ELEMENTI PRESENTI SULLA DIAGONALE PRINCIPALE E SUCCESSIVAMENTE SOMMARLI.

void seq_sum(double *matrix, int dim){
    
    //Dichiarazione variabili.
    int i;
    double *vector;
    double t0, t1, ts0, ts1, final_t, final_s, sum=0.0;
    ...
    for(i=0; i<dim; i++){
        ...
        sum += matrix[i+i*dim]; //valutare in caso non ci sia un numero divisibile per t.
    }
    ts1 = omp_get_wtime();
    final_s = ts1-ts0;
